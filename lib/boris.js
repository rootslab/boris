...
exports.Boris = ( function () {
    var log = console.log
        ...
        , collectResult = function ( rule, error, data, index, stripe ) {
            ...
            if ( slen === 0 ) {
                // check data, "*0\r\n" returns [], "*-1\r\n" returns [ null ]
                me.emit( 'match', error, data && data.length ? [ data ] : [], reveal );
                return;
            }
            ...
            while ( slen ) {
                ...
                // debug( 'pop' );
                if ( --slen === 0 ) {
                    me.emit( 'match', false, head.list, reveal );
                    break;
                }
            };
        }
...
, hat = function ( arr, len ) {
    ...
    me.arr = arr;
        ...
        // get results
        , emitResult = function ( rule, error, data, index, stripe ) {
            var me = this
                , arr = null
                , peela = me.peela
                , head = peela.head()
            ...
            if ( rule.isMultiBulk && IaN ) {
                ...
                if ( slen ) {
                    ...
                    if ( data > 3 ) me.emit( 'nut', false, arr, reveal );
                }
                peela.push( {
                    list : arr
                    , left : data
